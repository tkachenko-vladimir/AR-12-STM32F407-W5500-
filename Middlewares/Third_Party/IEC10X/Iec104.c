...
uint8_t IEC104_BuildClockACK(uint8_t qoi,uint8_t Prio){
...
uint8_t IEC104_ASDU_Call(PIEC10X_ASDU_T Iec10x_Asdu){
    ...
    switch((Iec10x_Asdu->_reason._reason) & 0xFF){
...
//            IEC104_BuildDetect(qoi ,IEC10X_M_ME_NC_1,Prio, ENDDEVICE_TYPE_HXTM);
            /**/
						IEC104_BuildSignal(qoi,Prio, ENDDEVICE_TYPE_HXGF);
            IEC104_BuildDetect(qoi ,IEC10X_M_ME_NC_1,Prio, ENDDEVICE_TYPE_HXGF);
...
uint8_t Iec104_Asdu_Clock(PIEC10X_ASDU_T Iec10x_Asdu){
                ...
                case IEC10X_C_SE_NC_1://50
                    if(Sq == 1){
                        ...
                        for(i=0; i<n; i++){
                            ...
                            ptr += sizeof(IEC10X_DETECT_F_T);
                            IEC10X->SetConfigF(Value, InfoAddr+i);
                        }
                    }else if(Sq == 0){
                        ptr = Iec10x_Asdu->_info;
                        for(i=0; i<n; i++){
                            ...
                            IEC10X->SetConfigF(Value, InfoAddr);
...
//            IEC104_BuildUpload(IEC10X_TI_AP_BASE_INFO, IEC10X_PRIO_INITLINK, AP_TYPE_BASE_INFO);
    ...
    if(IEC104_STATE_FLAG_INIT == IEC104_FLAG_CONNECT_SUCESS){
        ...
        /* test spon */
#if 0
        if(TestCount_Temp++>(10*30)){
            TestCount_Temp = 0;
//            IEC104_BuildSignal_Spon(1, TstValue, IEC104_INFOADDR_STATE_HXGF+2);
            IEC104_BuildDetectF_Spont(1, (float)TstValue, IEC104_INFOADDR_VALUE_HXTM+2);
            IEC104_BuildDetectF_Spont(0, (float)TstValue, IEC104_INFOADDR_VALUE_HXTM+3);
					TstValue++;
